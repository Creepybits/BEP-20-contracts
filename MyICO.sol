...
contract MyICOSaleDeployer {
    ...
    constructor(
        ...
        public
    {
        // Create the MyICO and keep its address:
        MyICO token = new MyICO(name, symbol, 0);
        token_address = address(token);

        // Create the MyICOSale and tell it about the token, set the goal, and set the open and close times to now and (now + 24 weeks):
        MyICOSale token_sale = new MyICOSale(1, wallet, token, goal, now, now + 24 weeks);
        token_sale_address = address(token_sale);

        // Make the MyICOSale contract a minter, then have the MyICOSaleDeployer renounce its minter role:
